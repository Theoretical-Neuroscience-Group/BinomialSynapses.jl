...
function run!(sim::NestedFilterSimulation; T::Int, plot_each_timestep = false, protocol = "exponential", parameter = 0.121, record_results = false, penalty = 0)
    ...
    for i in 1:T
        ...
        if record_results
            ...
            if i == T
                #if protocol == "uniform"
                #    save(string(Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]),"_",string(round(parameter[end],digits=5)),".jld"), "entropies", results.entropies, "dt", results.dt)
                #else
                #    save(string(Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]),"_",string(parameter),".jld"), "entropies", results.entropies, "dt", results.dt)
                #end      
                save(string(Base.parse(Int, ENV["SLURM_ARRAY_TASK_ID"]),".jld"), "entropies", results.entropies, "dt", results.dt)
            end
        end
    end
    return times, epsps
...
function OED_exact(sim::NestedFilterSimulation, deltat_candidates, times, i)
    ...
    for kk in 1:length(deltat_candidates)
        ...
        for l in 1:5
            ...
            p_star = prng[pind[idx]]
            q_star = qrng[qind[idx]]
            sigma_star = σrng[σind[idx]]
            tau_star = τrng[τind[idx]]
            
            x = 1
            var_n = 0
            var_n = p_star*(1-p_star)*N_star*x+(1-p_star)^2*var_n
            x = 1-(1-(1-p_star)*x)
            if i>1
                for ii in 2:i
            ...
            I = 1-exp(-deltat_candidates[kk]/tau_star)
            var_n = I*(1-I)*(N_star-N_star*x+N_star*x*p_star)+(1-I)^2*(p_star*(1-p_star)*N_star*x+(1-p_star)^2*var_n)
            x_temp = 1-(1-(1-p_star)*x)*exp(-deltat_candidates[kk]/tau_star)
            ...
            for m in 1:5
                ...
                h_2[m] = entropy(τ_posterior/sum(τ_posterior))
            end
            h_1[l] = mean(h_2)
        end
        h[kk] = mean(h_1)
